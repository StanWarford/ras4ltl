% John Wiegley, Scott Staley, Stan Warford
...
\addbibresource{ras4ltl-paper.bib}
...
\markright{A Revised Axiomatic System for Linear Temporal Logic}

\title{A Revised Axiomatic System\\for Linear Temporal Logic \opt{complete}{(Complete)}}
...
[\textit{Needs revising}]\\

Topics to discuss in the Introdction:\\

\begin{itemize}

\item Publication of the ACM paper \cite{WVS-1} sparks interest in proving every theorem in the paper using an automated theorem prover, thus confirming the correctness of the theorems and the proofs.\\

\item This had been done earlier using ACL2, but the proofs of more than 100 additional theorems were not included in the work. A snapshot of the work  was  completed and published recently\cite{PP1}.
This task was repeated and completed by the first author use the Coq theorem prover and is presented here for the first time.\\

\item Following this result the authors decided to continue collaboration with further work on the CDS4LTL system including refinement of its axiomatic basis, impproved system structure, new theorem discovery and other topics.\\
\end{itemize}
Propositional calculus is a formal system of logic based on the unary operator negation $\neg$,
the binary operators conjunction $\land$, disjunction $\lor$, implies $\impl$,
...
Gries and Schneider \cite{Gries1995, Gries1995145} show that $\mathcal{E}$, also known as \textit{calculation logic}, has several advantages over traditional logic systems.
The primary advantage of $\mathcal{E}$ over $\mathcal{H}$ systems is that calculation logic has only four proof rules, with inference rule Leibniz as the primary one.
...
Using calculation logic as a tool, LADM brings all the advantages of $\mathcal{E}$ to these additional knowledge domains.

The paper, A Calculational Deductive System for Linear Temporal Logic (CDS4LTL) \cite{Warford}, extends the calculation logic of LADM to linear temporal logic (LTL).
It surveys the LTL literature and presents all the theorems from the survey, plus many new ones, in a calculation logic system.
This paper improves on the CDS4LTL system by decreasing the number of axioms while at the same time using two newly-discovered axioms as its basis.
It presents additional theorems not in CDS4LTL and also extends the presentation of duality.

The following sections duplicate CDS4LTL to a large extent but omit details of the survey.
...
Section 3 presents the revised calculation logic system for linear temporal logic.
Section 4 summarizes previous linear temporal logic axiomatization systems and compares them with the current work.
[\textit{Revise this sentence}]

\section{LTL and CDS4LTL}

\section{Verification Tools}

[\textit{To be written.}]

\subsection{ACL2 Verification Work (2015)}

[\textit{To be written.}]

\subsection{Coq Verification Work (2021)}

[\textit{To be written.}]

\section{New Axiomatic Basis}
[\textit{To be written.}]

\begin{itemize}

\item Process of construction. What tools and analytical techniques were used? How were they used?
\item Results and impact on system structure. The point here is how much or how little change to the original CDS4LTL system was required? 
\item What can this work tell us about system maintenance and incremental extension?
\end{itemize}

\section{Metatheorems and Duality}

[\textit{To be written.}]

\section{Revised Theorem Ordering}
...
\section{Proofs by Hand vs. Proof Automation}

[\textit{To be written.}]

\section{Teaching Theorem Proving}
...
\section{Appendix A: Revised Equation Sheet}
...
\subsubsection{Propositional and temporal operators}
...
The inference rules for calculation logic are Substitution, Leibniz, Equanimity, and Transitivity.
\[
...
The predicate calculus in calculation logic has a consistent quantification notation that applies to Abelian monoids in both mathematics and logic.
...
\begin{align*}
\mathit{true} &\textrm{ and } \mathit{false}\\
\land         &\textrm{ and } \lor\\
\equiv        &\textrm{ and } \mynequiv\\
\impl         &\textrm{ and } \nfoll\\
\foll         &\textrm{ and } \nimpl
\end{align*}
...
Linear temporal logic extends the definition of $P_D$ for the temporal operators to include interchanging occurrences of $\Next$ and $\Next$ (self dual), and $\Always$ and $\Event$.
It is not the case that $\Wait$ is the dual of $\Until$.
Instead, each is the dual of one of the following two operators.
\begin{equation*}\label{EA:weakRelease}
\textbf{Definition of Weak Release $\mathcal{R}$:}\quad p\; \mathcal{R}\; q \equiv q \Wait(p\land q)
\end{equation*}
\begin{equation*}\label{EA:strongRelease}
\textbf{Definition of Strong Release $\mathcal{M}$:}\quad p\; \mathcal{M}\; q \equiv q \Until(p\land q)
\end{equation*}
With these definitions, the \textit{dual} $P_D$ of a LTL expression $P$ is constructed from $P$ by interchanging occurrences of:
\begin{align*}
\mathit{true} &\textrm{ and } \mathit{false}\\
\land         &\textrm{ and } \lor\\
\equiv        &\textrm{ and } \mynequiv\\
...
\Always       &\textrm{ and } \Event\\
\Until        &\textrm{ and } \mathcal{R}\\
\Wait         &\textrm{ and } \mathcal{M}
\end{align*}

\section{The Revised Temporal System}

This section presents the revised axiomatic deductive system of temporal logic and proves its theorems with the calculation
...
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  ...
  \> $=$  \>  \Hint{(3.47b) De Morgan, $\neg (p \lor q) \equiv \neg p \land \neg q$}\\[\lgap]
  \> \>   $\neg\Next\neg p \land \neg\Next \neg q$\\[\lgap]
  \> $=$  \>  \Hint{(\ref{E:linearity}) Linearity, twice}\\[\lgap]
...
\pagebreak
\textbf{LTL Program property proof example for CS340d}

\begin{equation}\label{E:Artale-1}
\textbf{example:}\quad \Always ((p \impl  \Event q) \land (q \impl \Next r) \land (r \impl \Event \Always s)) \impl (\Always p \impl \Event s)
\end{equation}
\opt{complete}{
\emph{Proof}:
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
  \> \>   $ \Always ( (p \impl  \Event q) \land (q \impl \Next r) \land (r \impl \Event \Always s)) \impl (\Always p \impl \Event s)$\\[\lgap]
   \> $=$  \>  \Hint{(\ref{E:distAlwaysAnd}) Distributivity of $\Always$ over $\land$}\\[\lgap]
  \> \>   $ \Always (p \impl  \Event q) \land \Always (q \impl \Next r) \land \Always (r \impl \Event \Always s) \impl (\Always p \impl \Event s)$\\[\lgap]
    \> $=$  \>  \Hint{(3.65) Shunting, $p\land q\impl r\equivs p\impl (q\impl r)$}\\[\lgap]
   \> \>   $ \Always (p \impl  \Event q) \land \Always (q \impl \Next r) \land \Always (r \impl \Event \Always s) \land \Always p \impl \Event s$\\[\lgap]
\end{tabbing}
And now,
\begin{tabbing}
\hspace{\mymathindent} \= $= \;$ \= \myqedtab \= \kill
   \> \>   $ \Always (p \impl  \Event q) \land \Always (q \impl \Next r) \land \Always (r \impl \Event \Always s) \land \Always p$\\[\lgap]
   \> $\impl$  \>  \Hintfirst{(\ref{E:nextEvent}) Weakening of $\Event$ and (3.82a) Transitivity and}\\[\lgap]
     \>  \>  \Hintlast{(\ref{E:distAlwaysImp}) Monotonicity of $\Always$ and (4.3) Monotonicity of $\land$}\\[\lgap]
   \> \>   $ \Always (p \impl  \Event q) \land \Always (q \impl \Event r) \land \Always (r \impl \Event \Always s) \land \Always p$\\[\lgap]
   \> $\impl$ \> \Hint{(\ref{E:EventCatRule}) Catenation rule of $\Event$ and (4.3) Monotonicity of $\land$} \\[\lgap]
    \> \>   $(p \impl  \Event r) \land \Always (r \impl \Event \Always s) \land \Always p$\\[\lgap]
     \> $\impl$  \>  \Hint{(\ref{E:impAlways}) Strengthening of $\Always$ and (4.3) Monotonicity of $\land$}\\[\lgap]
         \> \>   $(p \impl  \Event r) \land \Always (r \impl \Event \Always s) \land p$\\[\lgap]
      \> $=$ \> \Hint{(3.36) Symmetry of $\land$, $p\land q \equiv q\land p$, twice} \\[\lgap]
        \> \>   $p \land (p \impl  \Event r) \land \Always (r \impl \Event \Always s)$\\[\lgap]
 \> $\impl$  \>  \Hint{(3.77) Modus ponens, $p\land (p\impl q)\impl q$ and (4.3) Monotonicity of $\land$}\\[\lgap]
       \> \>   $\Event r \land \Always (r \impl \Event \Always s)$\\[\lgap]
        \> $\impl$ \> \Hint{(\ref{E:alwaysImpEvents}) Monotonicity of $\Event$ and (4.3) Monotonicity of $\land$} \\[\lgap]
  \> \>   $\Event r \land (\Event r \impl \Event \Event \Always s)$\\[\lgap]
 \> $=$  \>  \Hint{(\ref{E:IdemEvent}) Absorption of $\Event$}\\[\lgap]
  \> \>   $\Event r \land (\Event r \impl \Event \Always s)$\\[\lgap]
  \> $\impl$  \>  \Hint{(3.77) Modus ponens, $p\land (p\impl q)\impl q$}\\[\lgap]
    \> \>   $\Event \Always s$\\[\lgap]
   \> $\impl$ \> \Hintfirst{(\ref{E:impAlwaysE}) Strengthening of $\Always$ and (\ref{E:alwaysImpEvents}) Monotonicity of $\Event$ }\\[\lgap]
  \> $$ \> \Hintlast{ and (\ref{E:IdemEvent}) Absorption of $\Event$} \\[\lgap]
  \> \>   $\Event s$\quad \myqed
\end{tabbing}
}
...
\section{Modal logic}\label{modal-logic}

Linear temporal logic is just one of many different modal logic systems.
This section shows how linear temporal logic fits in general systems of modal logic and how calculation logic applies to modal logic.
...
Many users of LTL are concerned with the scalability of reasoning and automated theorem proving.
In contrast, the goal of this project is to make manual proofs accessible to human users.
It would be an interesting area for future research to determine if and/or how the calculational system might be applied to LTL synthesis.
